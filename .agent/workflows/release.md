...
12. **【必须】同步更新移动端版本号**（`mobile/pubspec.yaml`）：
...
13. 暂存并提交所有改动（包括文案与版本号）：
```bash
git add -A
git commit -m "vX.Y.Z: 简要描述本次更新内容"
```

14. 打 git tag（如果版本号不变则强制覆盖旧 tag）：
```bash
# 新版本号：
git tag vX.Y.Z
...
16. **提交并推送移动端仓库**（确保 CI 能拿到新的 versionCode）：
```bash
cd mobile
git add -A
...
17. **桌面端 Release 创建成功后**，触发移动端 CI 构建 APK 并上传到同一 Release：
```bash
...
18. 去 [Releases 页面](https://github.com/YuanShiJiLoong/author/releases) 确认：
...
19. （可选）下载 `.exe` 发到 QQ 群，或直接分享 Release 链接

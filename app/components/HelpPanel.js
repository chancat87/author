...
| **模型选择** | 点击「从 API 拉取模型列表」自动更新当前可用的大模型库供你选择 |
...
- 调试日志位于 \`%APPDATA%\\Author\\author-debug.log\`

### 故障诊断日志
如果使用时出现白屏、卡死或崩溃：
- 打开 **帮助 → 关于 → 导出诊断日志**，会下载一个 \`author-diagnostic-*.json\`
- 崩溃错误页也会显示 **导出诊断日志** 按钮
- 如果桌面客户端已经崩溃退出，会自动写入 \`%APPDATA%\\Author\\crash-reports\\author-crash-*.json\`，崩溃弹窗可直接打开日志目录
- 日志会包含最近的错误、警告、点击/拖拽等操作线索和桌面客户端主进程日志
- API Key、Token、Authorization 等敏感字段会自动脱敏
...
Author 支持基于 Firebase 的多设备实时云同步，让你在不同设备上无缝切换创作。同步范围采用隐私优先 allowlist。
